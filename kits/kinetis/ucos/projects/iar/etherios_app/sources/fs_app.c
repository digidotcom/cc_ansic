...
CPU_BOOLEAN  App_FS_Init (void)
{
    ...
    APP_TRACE_DEBUG(("\r\n"));
    APP_TRACE_DEBUG(("===================================================================\r\n"));
    APP_TRACE_DEBUG(("=                        FS INITIALIZATION                        =\r\n"));
    APP_TRACE_DEBUG(("===================================================================\r\n"));
    APP_TRACE_DEBUG(("Initializing FS...\r\n"));
    err = FS_Init((FS_CFG *)&App_FS_Cfg);
    if (err != FS_ERR_NONE) {
        APP_TRACE_DEBUG(("...init failed w/err = %d\r\n\r\n", err));
        return (DEF_FAIL);
    }
    ...
    APP_TRACE_DEBUG(("...init succeeded.\r\n"));
    APP_TRACE_DEBUG(("===================================================================\r\n"));
    APP_TRACE_DEBUG(("===================================================================\r\n"));
    APP_TRACE_DEBUG(("\r\n"));
...
static  CPU_BOOLEAN  App_FS_AddMSC (void)
{
    ...
    APP_TRACE_DEBUG(("    ===========================================================    \r\n"));
    APP_TRACE_DEBUG(("    Adding MSC device driver ...\r\n"));
    FS_DevDrvAdd((FS_DEV_API *)&FSDev_MSC,                      /* Add MSC device driver (see Note #1).                 */
                 (FS_ERR     *)&err);
    if (err != FS_ERR_NONE) {
        APP_TRACE_DEBUG(("    ... could not add MSC driver w/err = %d\r\n\r\n", err));
        return (DEF_FAIL);
    }
    return (DEF_OK);
}
...
static  CPU_BOOLEAN  App_FS_AddIDE (void)
{
    ...
    APP_TRACE_DEBUG(("    ===========================================================    \r\n"));
    APP_TRACE_DEBUG(("    Adding/opening IDE volume \"ide:0:\"...\r\n"));

    FS_DevDrvAdd((FS_DEV_API *)&FSDev_IDE,                      /* Add IDE/CF device driver.                            */
                 (FS_ERR     *)&err);
    if ((err != FS_ERR_NONE) &&
        (err != FS_ERR_DEV_DRV_ALREADY_ADDED)) {
        APP_TRACE_DEBUG(("    ...could not add driver w/err = %d\r\n\r\n", err));
        return (DEF_FAIL);
    }
    ...
    switch (err) {
        ...
        case FS_ERR_DEV_NOT_PRESENT:
             APP_TRACE_DEBUG(("    ...opened device (not present).\r\n"));
             return (DEF_FAIL);


        default:
             APP_TRACE_DEBUG(("    ...opening device failed w/err = %d.\r\n\r\n", err));
             return (DEF_FAIL);
    }
    ...
    switch (err) {
        case FS_ERR_NONE:
             APP_TRACE_DEBUG(("    ...opened volume (mounted).\r\n"));
             break;
        ...
        case FS_ERR_PARTITION_NOT_FOUND:                        /* Volume error (see Note #2).                          */
             APP_TRACE_DEBUG(("    ...opened volume (unmounted).\r\n"));
             return (DEF_FAIL);


        default:
             APP_TRACE_DEBUG(("    ...opening volume failed w/err = %d.\r\n\r\n", err));
             return (DEF_FAIL);
    }
...
static  CPU_BOOLEAN  App_FS_AddNAND (void)
{
    ...
    APP_TRACE_DEBUG(("    ===========================================================    \r\n"));
    APP_TRACE_DEBUG(("    Adding/opening NAND volume \"nand:0:\"...\r\n"));
    ...
    switch (err) {
        ...
        case FS_ERR_DEV_INVALID_LOW_FMT:                        /* Low fmt invalid (see Note #1b).                      */
             APP_TRACE_DEBUG(("    ...opened device (not low-level formatted).\r\n"));
#if (FS_CFG_RD_ONLY_EN == DEF_DISABLED)
             FS_NAND_LowFmt("nand:0:", &err);
#endif
             if (err != FS_ERR_NONE) {
                APP_TRACE_DEBUG(("    ...low-level format failed w/err = %d.\r\n", err));
                return (DEF_FAIL);
             }
             break;
    ...
    switch (err) {
        ...
        case FS_ERR_PARTITION_NOT_FOUND:                        /* Volume error (see Note #2).                          */
             APP_TRACE_DEBUG(("    ...opened device (not formatted).\r\n"));
#if (FS_CFG_RD_ONLY_EN == DEF_DISABLED)
             FSVol_Fmt("nand:0:", (void *)0, &err);
#endif
             if (err != FS_ERR_NONE) {
                APP_TRACE_DEBUG(("    ...format failed w/err = %d.\r\n", err));
                return (DEF_FAIL);
             }
             break;
        ...
        case FS_ERR_DEV_TIMEOUT:
        case FS_ERR_DEV_NOT_PRESENT:
             APP_TRACE_DEBUG(("    ...opened volume (unmounted) w/err = %d.\r\n", err));
             return (DEF_FAIL);
...
static  CPU_BOOLEAN  App_FS_AddNOR (void)
{
    ...
    APP_TRACE_DEBUG(("    ===========================================================    \r\n"));
    APP_TRACE_DEBUG(("    Adding/opening NOR volume \"nor:0:\"...\r\n"));
...
#endif
             if (err != FS_ERR_NONE) {
                APP_TRACE_DEBUG(("    ...low-level format failed.\r\n"));
                return (DEF_FAIL);
             }
             break;
...
static  CPU_BOOLEAN  App_FS_AddRAM (void)
{
    ...
    APP_TRACE_DEBUG(("    ===========================================================    \r\n"));
    APP_TRACE_DEBUG(("    Adding/opening RAM disk volume \"ram:0:\"...\r\n"));
...
static  CPU_BOOLEAN  App_FS_AddSD_Card (void)
{
    ...
    APP_TRACE_DEBUG(("    ===========================================================    \r\n"));
    APP_TRACE_DEBUG(("    Adding/opening SD/MMC (CardMode) volume \"sdcard:0:\"...\r\n"));
...
static  CPU_BOOLEAN  App_FS_AddSD_SPI (void)
{
    ...
    APP_TRACE_DEBUG(("    ===========================================================    \r\n"));
    APP_TRACE_DEBUG(("    Adding/opening SD/MMC (SPI) volume \"sd:0:\"...\r\n"));
