...
*
*           (4) Wright/Stevens, TCP/IP Illustrated, Volume 2, 3rd Printing, Section 25.1 'A FIN_WAIT_2
*               timer', Page 818 states that "to avoid leaving a connection   in the FIN_WAIT_2 state
*               forever ... when a connection moves from the FIN_WAIT_1 state to the FIN_WAIT_2 state
*               ... [the TCP connection's] timer is set to 10 minutes ... [and] 75 seconds, and when
*               it expires ... the connection is dropped".
*
*               See also 'net_tcp.h  TCP CONNECTION TIMEOUT DEFINES     Note #3'
*                      & 'net_tcp.c  NetTCP_RxPktConnHandlerFinWait1()  Note #3c'.
*
*           (5) RFC #2581, Section 4.2 states that "an ACK ... MUST be generated within 500 ms of the
...
*
*           (6) Configure timeout values in integer number of milliseconds.  Timeout values may also
...
#define  NET_TCP_CFG_RX_WIN_SIZE_OCTET                  4096u   /* Configure TCP connection receive  window size.       */
#define  NET_TCP_CFG_TX_WIN_SIZE_OCTET                  4096u   /* Configure TCP connection transmit window size.       */
...
#define  NET_TCP_CFG_TIMEOUT_CONN_FIN_WAIT_2_SEC          15u   /* ... timeout value, in integer seconds (see Note #4). */

                                                                /* Configure TCP acknowledgement delay ...              */
#define  NET_TCP_CFG_TIMEOUT_CONN_ACK_DLY_MS             500u   /* ... in integer milliseconds (see Note #5).           */
...
*
*           (4) Stevens, TCP/IP Illustrated, Volume 1, 8th Printing, Section 20.4, Page 282 states that
*               "4.2BSD defaulted the [socket] send and receive buffer to 2048 bytes each.  With 4.3BSD
*               both were increased to 4096 bytes ... Other systems, such as ... 4.4BSD ... use larger
*               default buffer sizes, such as 8192 or 16384 bytes".
*
...
#define  NET_SOCK_CFG_PORT_NBR_RANDOM_BASE             65000u   /* Configure random ports' starting port number.        */
                                                                ...
                                                                /* Configure (datagram) sockets' buffer sizes ...       */
                                                                /* ... in integer number of octets (see Note #4) :      */
#define  NET_SOCK_CFG_RX_Q_SIZE_OCTET                   4096    /* Configure socket receive  queue buffer size.         */
#define  NET_SOCK_CFG_TX_Q_SIZE_OCTET                   4096    /* Configure socket transmit queue buffer size.         */
...
*
* Note(s) : (1) Configure NET_SECURE_CFG_EN    to enable/disable network security manager.
*********************************************************************************************************
*/
                                                                /* Configure network security manager (see Note #1) :   */
#define  NET_SECURE_CFG_EN                      DEF_DISABLED

                                                                /* Configure total number of server secure sockets.     */
#define  NET_SECURE_CFG_MAX_NBR_SOCK_SERVER                5u

                                                                /* Configure total number of client secure sockets.     */
#define  NET_SECURE_CFG_MAX_NBR_SOCK_CLIENT                5u

                                                                /* Configure max length (in octets) of Server ...       */
#define  NET_SECURE_CFG_MAX_CERT_LEN                    1500u   /* .. certificates.                                     */

                                                                /* Configure max length (in octets) of Server keys.     */
#define  NET_SECURE_CFG_MAX_KEY_LEN                     1500u
...
#define  NET_SECURE_CFG_MAX_CA_CERT_LEN                 1500u   /* ... certificate authority certificates.              */
