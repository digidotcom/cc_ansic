 ...
 *      -# @ref initiate_config_to_cloud "Initiate Config" to Device Cloud to configure transport specific information and identity
 ...
 *
 *
 * @section config_request Configure Device Cloud
 *
 * A device can send a @ref initiate_config_to_cloud "Configure request to Device Cloud" at any
 * time.  The purpose is is used to configure transport specific information and identity information so that the server 
 * and device can communicate via SM.  The contents of the configure command are both transport specific and server/device specific.
 *
 * @subsection initiate_config_to_cloud  Initiate Configure Device Cloud
 *
 * When Configure is sent from the device to the server, information about the identity of the device is paired with transport 
 * specific information (such as phone number) that allows the server to communicate with the device.
 *
 * The application initiates a Configure Device Cloud by calling @ref connector_initiate_action()
 * using a @ref connector_initiate_config_request type and a connector_sm_send_config_request_t
 * request_data structure:
 *
 * @htmlonly
 * <table class="apitable">
 ...
 * <tr>
 *   <td>request</td>
 *   <td>@endhtmlonly @ref connector_initiate_config_request @htmlonly</td>
 * </tr>
 * <tr>
 *   <td>request_data</td>
 *   <td>Pointer to @endhtmlonly connector_sm_send_config_request_t @htmlonly structure, where member:
 *      <ul>
 *        <li><b><i>transport</i></b>: @endhtmlonly Should be set to @ref connector_transport_udp. @htmlonly </li>
 *        <li><b><i>user_context</i></b>: An opaque application-defined context.  This pointer is returned
 *                                        during a @endhtmlonly @ref config_response_callback "response callback". @htmlonly </li>
 *        <li><b><i>sim_slot</i></b>:single byte indicating the SIM slot: 0 means unknown or N/A, 1 is the first SIM slot, 
 *                                       2 is the second SIM slot, All other values reserved</li>
 *        <li><b><i>identifier</i></b>:null-terminated string containing an identifier for the current SIM. This will normally be
 *                                     set as the ICCID of the SIM.  If there is no SIM, an appropriate ID for this phone number 
 *                                     may be set instead. This field is optional</li>
 *        <li><b><i>response_required</i></b>: Set to @endhtmlonly @ref connector_true if notification of config receipt is needed,
 *                                             otherwise set to @ref connector_false.  See @ref config_response_callback . @htmlonly </li>
 *      </ul>
 *    </td>
 * </tr>
 * <tr><th class="subtitle">Values</th> <th class="subtitle">Description</th></tr>
 * <tr>
 *   <th>@endhtmlonly @ref connector_success @htmlonly</th>
 *   <td>Device Cloud Config initiated</td>
 * </tr>
 * <tr>
 *   <th>@endhtmlonly @ref connector_unavailable @htmlonly</th>
 *   <td>Cloud Connector is not running (not started or stopped) </td>
 ...
 *
 *
 * An example of initiating a config is shown below:
 * @code
 *   static connector_sm_send_config_request_t cfg_request;
 *   static char const identifier[] = "8934041511080745564";
 *
 *   cfg_request.transport = connector_transport_sms;
 *   cfg_request.response_required = connector_true;
 *   cfg_request.identifier = identifier;
 *   cfg_request.sim_slot = 0;
 *
 *   // Send config to Device Cloud
 *   status = connector_initiate_action(handle, connector_initiate_config_request, &cfg_request);
 * @endcode
 *
 * This example will invoke Cloud Connector to initiate a config request to Device Cloud. The completion
 * of this request is indicated by a @ref config_response_callback "response callback".
 *
 * @note The cfg_request variable must remain in scope or allocated until the @ref config_response_callback "response" callback. 
 *
 * @see @ref connector_initiate_action()
 * @see @ref config_response_callback
 *
 * @subsection config_response_callback  Config Device Cloud Response callback
 *
 * The config_response_callback is made for one of the following conditions: 
 * the config was successfully sent or a config response has been received
 * from the cloud, the config timed out, or an error occurred. 
 *
 * @note When @ref initiate_config_to_cloud operation has the response_required set @ref connector_false, this callback
 * is made with the @ref connector_sm_config_response_t status set to @b connector_sm_config_status_complete when the
 * message is sent.
 *
 * @htmlonly
 * <table class="apitable">
 ...
 * <tr>
 *   <td>request_id</td>
 *   <td>@endhtmlonly @ref connector_request_id_sm_config_response @htmlonly</td>
 * </tr>
 * <tr>
 *   <td>data</td>
 *   <td>Pointer to @endhtmlonly connector_sm_config_response_t @htmlonly structure:
 *     <ul>
 *       <li><b><i>transport</i></b>: @endhtmlonly Will be set to @ref connector_transport_sms. @htmlonly </li>
 *       <li><b><i>user_context</i></b>: The user_context pointer from the  @endhtmlonly @ref initiate_config_to_cloud. @htmlonly </li>
 *       <li><b><i>status</i></b>: The response code, where @endhtmlonly @b connector_sm_config_status_success indicates reply
 *                 received, @b connector_sm_config_status_complete indicates @ref initiate_config_to_cloud sent. @htmlonly </li>
 *     </ul>
 *   </td>
 * </tr>
 * <tr> <th colspan="2" class="title">Return Values</th> </tr>
 ...
 * @endhtmlonly
 *
 * @note The response code connector_sm_config_status_success will only be received if the @ref initiate_config_to_cloud request has the 
 * @b response_required set to @ref connector_true and a config response has been received from the cloud. 
 *
 * @see @ref initiate_config_to_cloud
 *
 *
 ...
 * notify an Application that a config was received.  The configure command is used to configure transport 
 * specific information and identity information so that the server and device can communicate via SM.  
 * The contents of the configure command are both transport specific and server/device specific.
 * When configure is sent from the server to a device, this indicates to the device the device settings 
 * needed to communicate with the server. These parameters are always transport specific. 
 *
 * @htmlonly
 * <table class="apitable">
 * <tr>
