 ...
 * @li @ref data_point_request : These are device originated messages to send data points to the iDigi Device Cloud.
 * The provided path name will be used as the stream ID and the stream can be accessed by the iDigi Device Manager.
 *
 ...
 * callback, the iDigi Connector will call the @ref total_length "total length" callback to get the total
 ...
 *            break;
 *
 *        case connector_data_service_type_have_data: // Response from the iDigi Device Cloud
 *            {
 ...
 *            break;
 *
 *        case connector_data_service_type_error: // The iDigi Device Cloud sent back an error
 *            {
 *                connector_data_service_block_t * const message = put_request->server_data;
 ...
 *
 * The data from the server (@ref connector_data_service_type_have_data message type) is in the 
 * connector_data_service_msg_request_t field, there is either data or an error. 
 * 
 * If the application needs to send back a response (@ref connector_data_service_type_need_data message type),
 * the application fills in the connector_data_service_msg_response_t.
 *
 * For a non-tcp transport session, before making the very first call to get the user response (only if
 * the iDigi Device Cloud requested for the response), the iDigi Connector will call the
 * @ref total_length "total length" callback to get the total length of the response data.
 *
 * The @ref connector_data_service_device_request "Device Request" @ref connector_callback_t "callback" 
 ...
 * <tr>
 *   <td>request_id</td>
 *   <td>@endhtmlonly @ref connector_data_service_device_request @htmlonly</td>
 * </tr>
 * <tr>
 *   <td>request_data</td>
 *   <td> [IN] pointer to @endhtmlonly connector_data_service_msg_request_t @htmlonly structure where:
 *     <b><i>service_context</i></b> is a pointer to @endhtmlonly @ref connector_data_service_device_request_t @htmlonly.<br></br>
 *     <b><i>message_type</i></b> can be set to one of the following:
 *       <ul>
 *       <li> @endhtmlonly @ref connector_data_service_type_have_data @htmlonly: data received from the iDigi Device Cloud.</li>
 *       <li> @endhtmlonly @ref connector_data_service_type_need_data @htmlonly: data for response to the iDigi Device Cloud.</li>
 *       <li> @endhtmlonly @ref connector_data_service_type_error @htmlonly: message processing error.</li>
 *       </ul>
 *     <b><i>server_data</i></b> is based on <i>message_type</i> setting:
 *       <ul>
 ...
 *   <li><b><i>message_status</i></b> is the callback status of @endhtmlonly @ref connector_msg_error_t @htmlonly type to send to the server on return.</li>
 *   <li><b><i>client_data</i></b> points to a @endhtmlonly connector_data_service_block_t @htmlonly structure where:
 *      <ul>
 ...
 *      <li><b><i>@endhtmlonly @ref data_service_flags "flags" @htmlonly</i></b> used to mark the @endhtmlonly @ref CONNECTOR_MSG_FIRST_DATA "first" @htmlonly data chunk, @endhtmlonly @ref  CONNECTOR_MSG_LAST_DATA "last" @htmlonly data chunk,
 *                              or to indicate the callback could not @endhtmlonly @ref  CONNECTOR_MSG_DATA_NOT_PROCESSED "complete" @htmlonly the message request.</li>
 *      </ul></li>
 *    </ul>
 * </td>
 * </tr>
 ...
 * {
 *     connector_data_service_device_request_t * server_device_request = request_data->service_context;
 ...
 *
 * The application initiates the Data point request to the iDigi Device Cloud by calling connector_initiate_action()
 ...
 *      <ul>
 *        <li><b><i>transport</i></b> is the communication method used to send the data point to the iDigi Device Cloud</li>
 ...
 * @endcode
 *
 * This example will invoke the iDigi Connector to initiate a data point request to the iDigi Device
 * Cloud. If the response needed is set (on non-TCP method), the application callback is called
 * with the cloud response or error in case the iDigi Connector fails to send the request.
 *
 * @subsection dp_response Callback with data point response
 *
 * After calling connector_initiate_action(), the iDigi Connector will prepare and send data point request
 * to the iDigi Device Cloud asynchronously. Upon receiving the response, it makes
