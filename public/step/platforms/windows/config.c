...
//#error "Specify device IP address. Set size to 4 (bytes) for IPv4 or 16 (bytes) for IPv6"
    ...
    if (app_get_edp_conn_ip_address(ip_address, size) == 0)
...
//#error "Specify device MAC address for LAN connection"
    ...
    static uint8_t const device_mac_addr[MAC_ADDR_LENGTH] = {0x00, 0x08, 0x74, 0xf2, 0x56, 0x80};
...
//#error  "Specify vendor id"
    ...
    static uint8_t const device_vendor_id[VENDOR_ID_LENGTH] = {0x03, 0x00, 0x00, 0x06};
...
static char const * app_data_service_class_to_string(connector_data_service_request_t const value)
{
    ...
    switch (value)
    {
        ...
        enum_to_case(connector_data_service_total_length);
        enum_to_case(connector_data_service_status);
        enum_to_case(connector_data_service_cli_request);
        enum_to_case(connector_data_service_dia_response);
        enum_to_case(connector_data_service_server_to_device_config);
        enum_to_case(connector_data_service_device_to_server_config);
    }
    return result;
}
