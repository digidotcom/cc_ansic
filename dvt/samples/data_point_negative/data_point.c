...
static void free_dp_single_ptr(connector_request_data_point_single_t * const dp_ptr)
{
    if (dp_ptr == NULL) goto done;
...
static connector_data_point_t * get_stream_points(connector_data_point_type_t const type)
{
    ...
    do
    {
        switch (type)
        {
            ...
            {
                static char * value[APP_MAX_POINTS] = {" ", "c,o,m,m,a", "line\nfeed", "t\ta\tb", "\"quote\"", "s p a c e"};
...
static void * get_data_point_single(dvt_dp_t * dvt_ptr)
{
    connector_request_data_point_single_t * dp_ptr = malloc(sizeof(connector_request_data_point_single_t));
    ...
    dp_ptr->type = dvt_ptr->data_type;
    dp_ptr->point = get_stream_points(dp_ptr->type);
    if (dp_ptr->point == NULL) goto error;

    {
        static char * units[] = {" ", "mph", "lb", NULL, ""};

        dp_ptr->unit = units[dp_ptr->type];
    }
    ...
    if (dp_ptr->transport != connector_transport_tcp)
    {
        ...
        if (dp_ptr->response_required)
            dp_ptr->timeout_in_seconds = CONNECTOR_SM_TIMEOUT_TX;
    }
...
connector_status_t app_send_data_point(connector_handle_t const handle)
{
    ...
    APP_DEBUG("Data point %s sent, status[%d]\n", ((connector_request_data_point_single_t *)(dvt_ptr->dp_ptr))->path, status);
    switch (status)
    {
        case connector_success:
            if ((transport == connector_transport_udp) && ((connector_request_data_point_single_t *)(dvt_ptr->dp_ptr))->response_required)
            {
...
connector_callback_status_t app_data_point_handler(connector_request_id_data_point_t const request_id, void * const data)
{
    ...
    switch (request_id)
    {
        ...
        case connector_request_id_data_point_single_response:
        {
            ...
            if (dp_ptr == NULL)
            {
                APP_DEBUG("Error: Received null single dp response context\n");
                status = connector_callback_error;
                goto error;
            }
        ...
        case connector_request_id_data_point_single_status:
        {
            ...
            if (dp_ptr == NULL)
            {
                APP_DEBUG("Error: Received null single dp status context\n");
                status = connector_callback_error;
                goto error;
            }
            ...
            dvt_ptr = &dvt_dp_info[dp_ptr->transport];
            APP_DEBUG("Received dp single status %d for %s\n", status_ptr->status, dp_ptr->path);
            free_dp_single_ptr(dp_ptr);
            break;
        }
