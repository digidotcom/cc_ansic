...
static firmware_list_t firmware_list[] = {
    ...
    {0x01000000, -1, ".*\\.*",        "any Image"}, /* any *.a files */
};
...
static connector_callback_status_t app_firmware_download_request(connector_fw_download_request_t const * const download_info, connector_fw_status_t * download_status)
{
    ...
    open_timing_table("Start firmware download", NULL);
...
static connector_callback_status_t app_firmware_download_complete(connector_fw_download_complete_request_t const * const complete_request, connector_fw_download_complete_response_t * complete_response)
{
...

    {
        ...
        static char fw_complete_text[sizeof (SIZE_TEXT MACRO_TO_STRING(SIZE_MAX))];

        sprintf(fw_complete_text, "%s%zu", SIZE_TEXT, total_image_size);

        close_timing_table("Complete firmware download", fw_complete_text);
    }
                                                  ...
                                                  void * response_data, size_t * const response_length)
{
    ...
    case connector_firmware_download_request:
        status = app_firmware_download_request((connector_fw_download_request_t *)request_data, (connector_fw_status_t *)response_data);
        break;

    case connector_firmware_binary_block:
        status = app_firmware_image_data((connector_fw_image_data_t *) request_data, (connector_fw_status_t *)response_data);
        break;

    case connector_firmware_download_complete:
        status = app_firmware_download_complete((connector_fw_download_complete_request_t *) request_data,
                                  (connector_fw_download_complete_response_t *) response_data);
        break;

    case connector_firmware_download_abort:
        status =  app_firmware_download_abort((connector_fw_download_abort_t *) request_data);
        break;

    case connector_firmware_target_reset:
        status =  app_firmware_reset((connector_fw_config_t *) request_data);
        break;
