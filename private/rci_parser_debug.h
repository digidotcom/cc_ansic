...
#define RCI_DEBUG_SHOW_ALL      connector_true
#define RCI_DEBUG_SHOW_DIFFS    connector_false
...
static char const * boolean_as_string(connector_bool_t const value)
{
...
static char const * connector_remote_config_request_t_as_string(connector_remote_config_request_t const value)
{
    char const * result;
    switch (value)
    {
...
static char const * connector_callback_status_t_as_string(connector_callback_status_t const value)
{
    char const * result;
    switch (value)
    {
        enum_to_case(connector_callback_continue);
        enum_to_case(connector_callback_busy);
        enum_to_case(connector_callback_abort);
        enum_to_case(connector_callback_unrecognized);
    }
    return result;
}
...
static char const * connector_remote_action_t_as_string(connector_remote_action_t const value)
{
    char const * result;
    switch (value)
    {
        enum_to_case(connector_remote_action_set);
        enum_to_case(connector_remote_action_query);
    }
    return result;
}

static char const * connector_remote_group_type_t_as_string(connector_remote_group_type_t const value)
{
    char const * result;
    switch (value)
    {
        enum_to_case(connector_remote_group_setting);
        enum_to_case(connector_remote_group_state);
    }
    return result;
}
...
static char const * connector_element_value_type_t_as_string(connector_element_value_type_t const value)
{
    char const * result;
    switch (value)
    {
#if defined RCI_PARSER_USES_STRING
        enum_to_case(connector_element_type_string);
#endif

#if defined RCI_PARSER_USES_MULTILINE_STRING
        enum_to_case(connector_element_type_multiline_string);
#endif
#if defined RCI_PARSER_USES_PASSWORD
        enum_to_case(connector_element_type_password);
#endif
#if defined RCI_PARSER_USES_INT32
        enum_to_case(connector_element_type_int32);
#endif
#if defined RCI_PARSER_USES_UINT32
        enum_to_case(connector_element_type_uint32);
#endif
#if defined RCI_PARSER_USES_HEX32
        enum_to_case(connector_element_type_hex32);
#endif
#if defined RCI_PARSER_USES_0X_HEX32
        enum_to_case(connector_element_type_0x_hex32);
#endif
#if defined RCI_PARSER_USES_FLOAT
        enum_to_case(connector_element_type_float);
#endif
#if defined RCI_PARSER_USES_ENUM
        enum_to_case(connector_element_type_enum);
#endif
#if defined RCI_PARSER_USES_ON_OFF
        enum_to_case(connector_element_type_on_off);
#endif
#if defined RCI_PARSER_USES_BOOLEAN
        enum_to_case(connector_element_type_boolean);
#endif
#if defined RCI_PARSER_USES_IPV4
        enum_to_case(connector_element_type_ipv4);
#endif
#if defined RCI_PARSER_USES_FQDNV4
        enum_to_case(connector_element_type_fqdnv4);
#endif
#if defined RCI_PARSER_USES_FQDNV6
        enum_to_case(connector_element_type_fqdnv6);
#endif
#if defined RCI_PARSER_USES_DATETIME
        enum_to_case(connector_element_type_datetime);
#endif
    }
    return result;
}
...
static void output_buffer_diff(char const * const name, void const * const current, void * const previous, size_t const bytes)
{
    if (memcmp(current, previous, bytes) != 0)
    {
        ...
        while (remaining != 0)
        {
            ...
            connector_debug_printf("%p: ", current_buffer);
            for (i = 0; i < bytes_this_line; i++)
            {
                unsigned char const byte = current_buffer[i];

                connector_debug_printf("%02x ", byte);
            }
            ...
            for (i = 0; i < bytes_this_line; i++)
            {
                ...
                connector_debug_printf("%c ", (isprint(byte) && !iscntrl(byte)) ? byte : replacement);
            }
...
static void output_debug_info(rci_t const * const current, connector_bool_t const show_all)
{
    ...
    connector_bool_t show_service_data = (current->service_data != NULL);
    ...
    connector_debug_printf("STEP %lu:\n", step);
    ...
    output_enum(connector_remote_config_request_t, callback.request.remote_config_request);
    output_enum(connector_callback_status_t, callback.status);
    ...
    /* TODO: shared.value */
    output_enum(connector_remote_action_t, shared.request.action);
    output_enum(connector_remote_group_type_t, shared.request.group.type);
