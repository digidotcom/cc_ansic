...
static connector_bool_t one_element_set(rci_t const * const rci)
{
    connector_bool_t const is_set = is_set_command(rci->input.command);
...
{
    connector_group_table_t const * const table = (connector_group_table + rci->shared.request.group.type);
    ...
    if (pending_rci_callback(rci))
    {
...

        {
            ...
            case connector_remote_config_group_end:
                {
                    connector_group_t const * const group = (table->groups + get_group_id(rci));
    ...
    case rci_traversal_state_one_element_start:
        {
            ...
            if (is_query_command(rci->input.command) && (element->access == connector_element_access_write_only))
            {
                switch (rci->traversal.state)
                {
                UNHANDLED_CASES_ARE_INVALID
    ...
    case rci_traversal_state_one_element_end:
        {
            connector_group_element_t const * const element = get_current_element(rci);
            connector_bool_t const reading_write_only = connector_bool(is_query_command(rci->input.command) && (element->access == connector_element_access_write_only));
