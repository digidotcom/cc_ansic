...
typedef struct
{
    void * handle;
    void * user_context;
    connector_file_error_data_t error;
    union
    {
        struct
        {
        ...
        struct
        {
            ...
            connector_file_hash_algorithm_t hash_alg;
        } d;
    }data;
...
#define CONNECTOR_FILE_TRUNC            0x01
#define CONNECTOR_FILE_ERROR_HINT       0x80

#define FILE_STATE_NONE             0
#define FILE_STATE_LSEEK1_DONE      1
#define FILE_STATE_LSEEK_DONE       2
...
#define FileIsDir(context)    FsIsBitSet(context->flags, CONNECTOR_FILE_IS_DIR)
#define FileIsReg(context)    FsIsBitSet(context->flags, CONNECTOR_FILE_IS_REG)

#define FileNeedTrunc(context)  FsIsBitSet(context->flags, CONNECTOR_FILE_TRUNC)
#define FileClearTrunc(context) FsBitClear(context->flags, CONNECTOR_FILE_TRUNC)

#define FileHasErrorHint(context) FsIsBitSet(context->flags, CONNECTOR_FILE_ERROR_HINT)
#define FileSetErrorHint(context) FsBitSet(context->flags, CONNECTOR_FILE_ERROR_HINT)

#define FileSetState(context, s) (context->state = s)
#define FileGetState(context)    (context->state)


#define fileOperationSuccess(status, context) (status==connector_working && context->error.error_status==connector_file_noerror)

static void set_file_system_service_error(msg_service_request_t * const service_request, connector_msg_error_t const msg_error)
{
...
static connector_status_t fs_set_abort(connector_data_t * const connector_ptr,
                                   ...
                                   connector_file_system_request_t const file_request,
                                  ...
                                  msg_service_request_t * const service_request)
{
     ...
     error_code -= connector_file_unspec_error;
     ...
     else
     {
        ...
        {
            fs_set_abort(connector_ptr, context, 
                                  connector_file_system_strerror, connector_invalid_data_size);
            goto done;
        }
        else
        {
...
static connector_status_t call_file_system_user(connector_data_t * const connector_ptr,
                                                    msg_service_request_t * const service_request,
                                                    connector_file_system_request_t const fs_request_id,
                                                    ...
                                                    size_t const response_length)
{
    msg_session_t * const session = service_request->session;
    file_system_context_t * const context = session->service_context;
    connector_file_response_t * const response = response_data;
    void const * const old_errnum = context->error.errnum;
    connector_msg_error_t  msg_error = connector_msg_error_none;
    ...
    switch (callback_status)
    {
        case connector_callback_continue:
            ...
            if (response_length != response_length_in)
            {
                ...
                fs_set_abort(connector_ptr, context, 
                             fs_request_id, connector_invalid_data_size);
            }
            else
            if (context->error.error_status == connector_file_user_cancel)
                msg_error = connector_msg_error_cancel;
            break;
    ...
    if (msg_error != connector_msg_error_none)
    {
        context->error.error_status = connector_file_user_cancel;
...
static size_t file_hash_size(connector_file_hash_algorithm_t const hash_alg)
{
    size_t result;
...
static connector_status_t call_file_stat_user(connector_data_t * const connector_ptr,
                                                   ...
                                                   connector_file_hash_algorithm_t const hash_alg)
{
    msg_session_t * const session = service_request->session;
    file_system_context_t * const context = session->service_context;
    connector_file_stat_request_t request;
    connector_file_stat_response_t response;
    connector_status_t status;

    response.statbuf.flags = 0;
    response.statbuf.hash_alg = connector_file_hash_none;

    request.path = path;
    request.hash_alg = hash_alg;

    status = call_file_system_user(connector_ptr, service_request, connector_file_system_stat,
    ...
    if (fileOperationSuccess(status, context))
    {
            ...
            case connector_file_hash_best:
                switch(response.statbuf.hash_alg)
                {
                    ...
                    case connector_file_hash_none:
                        ...
                        break;
                    default:
                        context->data.d.hash_alg = connector_file_hash_none;
                        break;
                }
                break;
        ...
        if (context->data.d.hash_alg != connector_file_hash_none)
        {
                                                       ...
                                                       char const * const path)
{
    msg_session_t * const session = service_request->session;
    file_system_context_t * const context = session->service_context;
    connector_file_path_request_t request;
    connector_file_open_response_t response;
    connector_status_t   status;

    request.path = path;
    response.handle = NULL;

    status = call_file_system_user(connector_ptr, service_request, connector_file_system_opendir,
                                    &request,  sizeof request,
                                    &response, sizeof response);
    ...
    if (fileOperationSuccess(status, context))
    {
        ...
        {
            status = fs_set_abort(connector_ptr, context, 
                         connector_file_system_opendir, connector_invalid_data);
        }
    }
                                                       ...
                                                       size_t const buffer_size)
{
    msg_session_t * const session = service_request->session;
    file_system_context_t * const context = session->service_context;
    connector_file_request_t request;
    connector_file_data_response_t response;
    connector_status_t  status;
    ...
    status = call_file_system_user(connector_ptr, service_request, connector_file_system_readdir,
                                    &request,  sizeof request,
                                    &response, sizeof response);
    if (fileOperationSuccess(status, context))
    {
        ...
        if ((response.size_in_bytes > buffer_size) || (path[response.size_in_bytes-1] != '\0'))
        {
            /* wrong size returned and let's cancel the request */
            status =  fs_set_abort(connector_ptr, context, 
                                   connector_file_system_readdir, connector_invalid_data_size);
            goto done;
        }
    }
done:
                                                  ...
                                                  connector_file_system_request_t const fs_request_id)
{
    ...
    if (context->handle != NULL)
    {
        connector_file_error_data_t const old_error_data = context->error;
        connector_file_request_t request;
        connector_file_response_t response;
        connector_status_t old_status = context->status;

        request.handle = context->handle;
        FileSetState(context, FILE_STATE_CLOSING);

        status = call_file_system_user(connector_ptr, service_request, fs_request_id,
                                        &request,  sizeof request,
                                        &response, sizeof response);

        /* Don't overwrite existing error status in close */
        if (old_error_data.error_status != connector_file_noerror)
        ...
        else
        {
            ...
            if (old_status != connector_working)
            {
                status = context->status = old_status;
            }
       }
    }
    return status;
                                                   ...
                                                   uint8_t * const hash_ptr)
{
    ...
    buffer_size = response.size_in_bytes = file_hash_size(context->data.d.hash_alg);
    ...
    status = call_file_system_user(connector_ptr, service_request, connector_file_system_hash,
                                   &request,  sizeof request,
                                   &response, sizeof response);
    ...
    if (response.size_in_bytes > buffer_size)
    {
        /* wrong size returned and let's cancel the request */
        status =  fs_set_abort(connector_ptr, context, 
                         connector_file_system_hash, connector_invalid_data_size);
        goto done;
    }
                                                   ...
                                                   int const oflag)
{
    ...
    connector_status_t      status;
    connector_file_open_request_t    request;
    connector_file_open_response_t   response;
    ...
    status = call_file_system_user(connector_ptr, service_request, connector_file_system_open,
                                   &request,  sizeof request,
                                   &response, sizeof response);
    ...
    if (fileOperationSuccess(status, context))
    {
        ...
        {
            status =  fs_set_abort(connector_ptr, context, 
                             connector_file_system_open, connector_invalid_data);
        }
    }
                                                     ...
                                                     long int * const offset_out)
{
    ...
    connector_status_t status;
    connector_file_lseek_request_t request;
    connector_file_lseek_response_t response;
    ...
    *offset_out = -1;
    ...
    status = call_file_system_user(connector_ptr, service_request, connector_file_system_lseek,
                                   &request, sizeof request,
                                   &response, sizeof response);
                                                        ...
                                                        msg_service_request_t * const service_request)
{
    ...
    file_system_context_t * const context = session->service_context;
    connector_status_t status;
    connector_file_ftruncate_request_t request;
    connector_file_response_t response;

    request.handle = context->handle;
    request.length = context->data.f.offset;

    status = call_file_system_user(connector_ptr, service_request, connector_file_system_ftruncate,
                                   &request, sizeof request,
                                   &response, sizeof response);
                                                 ...
                                                 char const * const path)
{
    ...
    status = call_file_system_user(connector_ptr, service_request, connector_file_system_rm,
                                   &request, sizeof request,
                                   &response, sizeof response);
                                                     ...
                                                     size_t * const buffer_size)
{
    ...
    status = call_file_system_user(connector_ptr, service_request, connector_file_system_read,
                                   &request, sizeof request,
                                   &response, sizeof response);
    ...
    if (response.size_in_bytes > *buffer_size)
    {
        ...
        status =  fs_set_abort(connector_ptr, context, 
                         connector_file_system_read, connector_invalid_data_size);
        goto done;
    }
                                                     ...
                                                     size_t * const bytes_done)
{
    ...
    file_system_context_t * const context = session->service_context;
    connector_status_t status;
    connector_file_write_request_t request;
    connector_file_write_response_t response;
    ...
    status = call_file_system_user(connector_ptr, service_request, connector_file_system_write,
                                   &request, sizeof request,
                                   &response, sizeof response);

    if (fileOperationSuccess(status, context))
    {
        if (response.size_in_bytes > request.size_in_bytes)
        {
            status =  fs_set_abort(connector_ptr, context, 
                             connector_file_system_write, connector_invalid_data_size);
            goto done;
        }
                              ...
                              size_t const buffer_size)
{
    ...
    if (path_len == 0)
    {
        context->error.error_status = connector_file_request_format_error;
                                                        ...
                                                        msg_service_request_t * const service_request)
{
        ...
        goto done;
    {
        ...
        status = call_file_open_user(connector_ptr, service_request, path, CONNECTOR_O_RDONLY);
    }
done:
    return status;
}
...

{
    ...
    if (FileGetState(context) < FILE_STATE_LSEEK1_DONE)
    {
        status = call_file_lseek_user(connector_ptr, service_request, 0, CONNECTOR_SEEK_END, &ret);
        if (status == connector_pending || !fileOperationSuccess(status, context))
            goto done;
    ...
    FileSetState(context, FILE_STATE_LSEEK1_DONE);

    status = call_file_lseek_user(connector_ptr, service_request, (long int ) context->data.f.offset, CONNECTOR_SEEK_SET, &ret);
    if (status == connector_pending || !fileOperationSuccess(status, context))
    {
        goto done;
    }
...
error:
    if (ret == -1)
    {
        context->error.error_status = connector_file_invalid_parameter;
                                                          ...
                                                          msg_service_request_t * const service_request)
{
    ...
    if ((context->error.error_status != connector_file_noerror) || (FileGetState(context) == FILE_STATE_CLOSING))
    {
...

    {
        ...
        {
           if (FileGetState(context) < FILE_STATE_LSEEK_DONE)
           {
               if (context->data.f.offset != 0)
               {
                    status = set_file_position(connector_ptr, service_request);
                    if (status == connector_pending)
                        goto done;
               ...
               FileSetState(context, FILE_STATE_LSEEK_DONE);
           }
        ...
        while (bytes_to_read > 0)
        {
            ...
            status = call_file_read_user(connector_ptr, service_request, data_ptr, &cnt);
...
close_file:
    if (context->handle != NULL)
    {
        status = call_file_close_user(connector_ptr, service_request, connector_file_system_close);
        if (status == connector_abort) 
            goto done;

        if (status == connector_pending)
        {
             if (service_data->length_in_bytes > 0)
             {
        ...
        (status == connector_working))
    {
        ...
        if (!MsgIsStart(service_data->flags))
            set_file_system_service_error(service_request, connector_msg_error_cancel);
        else
            if (format_file_error_msg(connector_ptr, service_request) == connector_abort)
                                                       ...
                                                       msg_service_request_t * const service_request)
{
    ...
    status = call_file_rm_user(connector_ptr, service_request, path);
                                                         ...
                                                         file_system_opcode_t const opcode)
{
    ...
    if (context->error.error_status == connector_file_noerror)
    {
                                                        ...
                                                        msg_service_request_t * const service_request)
{
...

    {
        ...
        if (MsgIsStart(service_data->flags))
        {
            ...
            if (context->handle == NULL)
            {
                int open_flags = CONNECTOR_O_WRONLY | CONNECTOR_O_CREAT;
                char const * path = service_data->data_ptr;
                path += FILE_OPCODE_BYTES;
                ...
                if (FileNeedTrunc(context))
                {
                    if (context->data.f.offset == 0)
                    {
                        open_flags |= CONNECTOR_O_TRUNC;
                        FileClearTrunc(context);
                    }
                }

                status = call_file_open_user(connector_ptr, service_request, path, open_flags);
                if (context->handle == NULL)
                    goto done;
            }
        ...
        while(bytes_to_write > 0)
        {
            ...
            status = call_file_write_user(connector_ptr, service_request, data_ptr, &cnt);
        ...
        if (FileNeedTrunc(context))
        {
            status = call_file_ftruncate_user(connector_ptr, service_request);
            if (status == connector_pending)
                goto done;
        }
    }
                                   ...
                                   size_t const buffer_size)
{
    ...
    if (len != 0)
    {
        ...
        hash_alg = (connector_file_hash_algorithm_t) *data_ptr;
...
static size_t format_file_ls_response_header(connector_file_hash_algorithm_t const hash_alg,
                                      ...
                                      uint8_t * const data_ptr)
{
    ...
    uint8_t const flags = FileIsDir(context) ? CONNECTOR_FILE_IS_DIR : 0;
    size_t result;
...
static connector_status_t file_store_path(file_system_context_t * const context,
                                               char const * const path)
{
    connector_status_t status = connector_working;
    ...
    {
        context->error.error_status = connector_file_out_of_memory;
                                                       ...
                                                       msg_service_request_t * const service_request)
{
    ...
    {
        status = call_file_stat_user(connector_ptr, service_request, path, context->data.d.hash_alg);
        if ((status == connector_pending) || !fileOperationSuccess(status, context))
            goto done;
    ...
    if (FileIsDir(context))
    {
        status = call_file_opendir_user(connector_ptr, service_request, path);
        if ((status == connector_pending) || !fileOperationSuccess(status, context))
            goto done;
    ...
    }
    {
        ...
        if (context->handle == NULL)
        {
            ...
            if (hash_len != 0)
            {
                ...
                status = call_file_hash_user(connector_ptr, service_request, file_path, hash_ptr);
                if (status == connector_pending)
                    goto done;
        ...
        else
        {
            ...
            while (FileGetState(context) < FILE_STATE_READDIR_DONE)
            {
                /* read next dir entry */
                ...
                status = call_file_readdir_user(connector_ptr, service_request, file_path, path_max);
                if (status == connector_pending)
                    goto done;
            ...
            if (FileGetState(context) < FILE_STATE_STAT_DONE)
            {
                status = call_file_stat_user(connector_ptr, service_request, context->data.d.path, connector_file_hash_none);
                if (status == connector_pending)
                    goto done;
...
close_dir:
    if (context->handle != NULL)
    {
        status = call_file_close_user(connector_ptr, service_request, connector_file_system_closedir);
        if (status == connector_abort) 
            goto done;
                                              ...
                                              file_system_context_t * * const result)
{
    ...
    context->error.error_status = connector_file_noerror;
                                                            ...
                                                            msg_service_request_t * const service_request)
{
    ...
    else
    {
        if (context == NULL)
        {
            set_file_system_service_error(service_request, connector_msg_error_format);
    ...
    switch (context->opcode)
    {
        case fs_get_request_opcode:
            status = process_file_get_request(connector_ptr, service_request);
            break;

        case fs_put_request_opcode:
            status = process_file_put_request(connector_ptr, service_request);
            break;

        case fs_rm_request_opcode:
            status = process_file_rm_request(connector_ptr, service_request);
            break;

        case fs_ls_request_opcode:
            status = process_file_ls_request(connector_ptr, service_request);
            break;
                                                             ...
                                                             msg_service_request_t * const service_request)
{
    ...
    if (context == NULL)
    {
        set_file_system_service_error(service_request, connector_msg_error_unknown_session);
        ASSERT_GOTO(connector_false, done);
    }
    ...
    {
        case fs_get_request_opcode:
            status = process_file_get_response(connector_ptr, service_request);
            break;

        case fs_put_request_opcode:
            status = process_file_response_nodata(connector_ptr, service_request, fs_put_response_opcode);
            break;

        case fs_rm_request_opcode:
            status = process_file_response_nodata(connector_ptr, service_request, fs_rm_response_opcode);
            break;

        case fs_ls_request_opcode:
            status = process_file_ls_response(connector_ptr, service_request);
            break;
...
static connector_status_t file_system_error_callback(connector_data_t * const connector_ptr,
                                                          msg_service_request_t * const service_request)
{
    msg_session_t * const session = service_request->session;
    file_system_context_t * const context = session->service_context;
    ...
    request_id.file_system_request = connector_file_system_msg_error;
    ...
    if (context != NULL)
    {
        ...
        if (context->handle != NULL)
        {
            connector_file_system_request_t fs_request_id = context->opcode == fs_ls_request_opcode ?
                                            connector_file_system_closedir : connector_file_system_close;

            status = call_file_close_user(connector_ptr, service_request, fs_request_id);
            response.user_context = context->user_context;
                                                    ...
                                                    msg_service_request_t * const service_request)
{
    ...
    case msg_service_type_error:
        status = file_system_error_callback(connector_ptr, service_request);
        break;
