...
static connector_element_value_enum_t on_off_enum = { asizeof(on_off_strings), on_off_strings};
#endif

#if defined RCI_PARSER_USES_BOOLEAN
static char const * const boolean_strings[] = {RCI_FALSE, RCI_TRUE};
static connector_element_value_enum_t boolean_enum = { asizeof(boolean_strings), boolean_strings};
#endif
...
static void rci_output_content(rci_t * const rci)
{
    ...
    (defined RCI_PARSER_USES_INT32) || (defined RCI_PARSER_USES_UINT32) || (defined RCI_PARSER_USES_HEX32) || (defined RCI_PARSER_USES_0X_HEX32) || \
...
static connector_bool_t rci_scan_formatted(char const * const input, char const * const format, ...)
{
    connector_bool_t error;

    va_list ap;

    va_start(ap, format);
    errno = 0;
    error = connector_bool((vsscanf(input, format, ap) != 1) || errno != 0);
    va_end(ap);
...
static connector_bool_t rci_scan_enum(char const * const input, connector_element_value_enum_t const * const match, unsigned int * const result)
{
    connector_bool_t error = connector_true;
    size_t i;
...
static void rci_handle_content(rci_t * const rci)
{
    ...
    if (element->access == connector_element_access_read_only)
    {
        goto done;
    }

    /* NUL-terminate the content as we know it is always followed by a '<' (or we have been compressing entities), so it is impossible to overrun the buffer. */
    {
    ...
    case connector_element_type_datetime:
        ...
        {
            connector_element_value_string_t const * const limit = &element->value_limit->string_value;
            error = connector_bool((string_length < limit->min_length_in_bytes) || (string_length > limit->max_length_in_bytes));
        }
        break;
    ...
    case connector_element_type_float:
        ...
        if (!error && (element->value_limit != NULL))
        {
            ...
            case connector_element_type_int32:
                {
                    connector_element_value_signed_integer_t const * const limit = &element->value_limit->signed_integer_value;
                    int32_t const value = rci->shared.value.signed_integer_value;

                    error = connector_bool((value < limit->min_value) || (value > limit->max_value));
                }
                break;
#endif
...
#endif
                {
                    connector_element_value_unsigned_integer_t const * const limit = &element->value_limit->unsigned_integer_value;
            ...
            case connector_element_type_float:
                {
                    connector_element_value_float_t const * const limit = &element->value_limit->float_value;
    ...
    if (error)
    {
        rci_global_error(rci, connector_rci_error_bad_value, RCI_NO_HINT);
    }
    else
    {
...
static void rci_process_group_tag(rci_t * const rci, rci_action_t const rci_action)
{
    ...
    {
        rci_global_error(rci, connector_rci_error_bad_group, RCI_NO_HINT);
        goto error;
    }

    {
        ...
        if (rcistr_valid(index))
        {
            ...
            if (!rcistr_to_uint(index, &group_index) || (group_index > group->instances))
            {
                ...
                rci_global_error(rci, connector_rci_error_bad_index, RCI_NO_HINT);
                goto error;
            }
    ...
    case rci_command_query_state:
        {
            ...
            if (element->access != connector_element_access_write_only)
            {
                rci->output.tag = rci->shared.string.tag;
                rci->output.type = rci_output_type_start_tag;
                state_call(rci, rci_parser_state_output);
...
static void rci_process_element_tag(rci_t * const rci, rci_action_t const rci_action)
{
    ...
    {
        rci_global_error(rci, connector_rci_error_bad_element, RCI_NO_HINT);
        goto error;
    }
...
static void rci_handle_unary_tag(rci_t * const rci)
{
    ...
    case rci_command_unseen:
        rci_global_error(rci, connector_rci_error_parser_error, RCI_NO_HINT);
        break;
    ...
    case rci_command_header:
        ...
        case rci_command_set_setting:
        case rci_command_set_state:
            rci_global_error(rci, connector_rci_error_bad_xml, RCI_NO_HINT);
            break;
        ...
        case rci_command_query_state:
            ...
            {
            UNHANDLED_CASES_ARE_INVALID
            case rci_command_query_setting: rci->shared.request.action = connector_remote_action_query; rci->shared.request.group.type = connector_remote_group_setting;    break;
            case rci_command_query_state:   rci->shared.request.action = connector_remote_action_query; rci->shared.request.group.type = connector_remote_group_state;      break;
            }
...
static void rci_handle_start_tag(rci_t * const rci)
{
    ...
    case rci_command_unseen:
        if (cstr_equals_rcistr(RCI_REQUEST, &rci->shared.string.tag))
        {
            ...
            {
                rci_global_error(rci, connector_rci_error_invalid_version, RCI_NO_HINT);
            }
        }
        else
        {
    ...
    case rci_command_header:
        ...
        else
        {
            ...
            {
            UNHANDLED_CASES_ARE_INVALID
            case rci_command_set_setting:   rci->shared.request.action = connector_remote_action_set;   rci->shared.request.group.type = connector_remote_group_setting;    break;
            case rci_command_set_state:     rci->shared.request.action = connector_remote_action_set;   rci->shared.request.group.type = connector_remote_group_state;      break;
            case rci_command_query_setting: rci->shared.request.action = connector_remote_action_query; rci->shared.request.group.type = connector_remote_group_setting;    break;
            case rci_command_query_state:   rci->shared.request.action = connector_remote_action_query; rci->shared.request.group.type = connector_remote_group_state;      break;
            }
...
static void rci_handle_end_tag(rci_t * const rci)
{
    ...
    {
        connector_remote_config_request_t config_request_id;

        if (have_group_id(rci))
        {
            config_request_id = connector_remote_config_group_end;
        }
        else if (rci->input.command != rci_command_header)
        {
            config_request_id = connector_remote_config_action_end;
        }
        else
        {
            config_request_id = connector_remote_config_session_end;
        }
...
static void rci_parse_input_semicolon(rci_t * const rci)
{
    ...
    case rci_input_state_content_escaping:
        {
            char const * const entity_start = rcistr_data(&rci->input.entity);
            
            end_rcistr(rci, &rci->input.entity);
            ASSERT(rcistr_char(&rci->input.entity, 0) == '&');
            rcistr_clip_left(&rci->input.entity, 1);

            {
                int const value = rci_entity_value(&rci->input.entity);
                
                if (value != nul)
                {
                    rci->input.character = value;
                    rci->input.destination = (char *) entity_start;
                }
            }
            
            clear_rcistr(&rci->input.entity);
            rci->input.state = rci_input_state_content;
            break;
        }
...
static void rci_parse_input(rci_t * const rci)
{
    ...
    if (pending_rci_callback(rci))
    {
...

        {
            ...
            case connector_remote_config_group_process:
                {
                    connector_remote_group_request_t const * const request = &rci->shared.request;
    ...
    while ((rci->parser.state.current == rci_parser_state_input) && (rci_buffer_remaining(input) != 0))
    {
        ...
        if (rci->callback.status == connector_callback_busy)
        {
            goto done;
        }
    }
