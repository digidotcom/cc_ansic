...
static void rci_generate_error(rci_t * const rci)
{
    ...
    case rci_error_state_element_close:
        {
            connector_group_table_t const * const table = (connector_group_table + rci->shared.request.group.type);
            connector_group_t const * const group = (table->groups + get_group_id(rci));
            connector_group_element_t const * const group_element = (group->elements.data + get_element_id(rci));
    ...
    case rci_error_state_group_close:
        if (remote_config_request != connector_remote_config_group_end)
        {
    ...
    case rci_error_state_command_close:
        if (remote_config_request != connector_remote_config_action_end)
        {
    ...
    case rci_error_state_reply_close:
        if (remote_config_request != connector_remote_config_session_end)
        {
