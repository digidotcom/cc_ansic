...
static connector_bool_t buffer_equals_buffer(char const * const str1, size_t const len1, char const * const str2, size_t const len2)
{
    return connector_bool((len1 == len2) && (memcmp(str1, str2, len1) == 0));
}

static connector_bool_t cstr_equals_buffer(cstr_t const * const cstr, char const * const str2, unsigned int const len2)
{
    return buffer_equals_buffer(cstr_data(cstr), cstr_len(cstr), str2, len2);
}

static connector_bool_t cstr_equals_rcistr(cstr_t const * const cstr, rcistr_t const * const rcistr)
{
    return cstr_equals_buffer(cstr, rcistr->data, rcistr->length);
}

static connector_bool_t rcistr_equals_rcistr(rcistr_t const * const one, rcistr_t const * const two)
{
...
static connector_bool_t cstr_equals_str(cstr_t const * const cstr, char const * const str)
{
    return cstr_equals_buffer(cstr, str, strlen(str));
}
#endif

static connector_bool_t rcistr_to_uint(rcistr_t const * const rcistr, unsigned int * const value)
{
    ASSERT(!isdigit(rcistr->data[rcistr->length]));

    return connector_bool(sscanf(rcistr->data, "%u", value) == 1);
}
...
static connector_bool_t rcistr_empty(rcistr_t const * const string)
{
...
static void rcistr_clip_left(rcistr_t * const string, size_t const length)
{
    ASSERT(string->length >= length);
    
    {
        size_t const distance = (string->length >= length) ? length : string->length;
