        ...
        case connector_sm_state_receive_data:
            if (session->timeout_in_seconds != SM_WAIT_FOREVER)
            {
                ...
                if (current_time > (session->start_time + session->timeout_in_seconds))
                {
                    ...
                    connector_debug_printf("Sm session [%u (ClientOwned=%u)]  timeout... start time:%u, current time:%u\n", session->request_id, SmIsClientOwned(session->flags), session->start_time, current_time);
                }
            }
