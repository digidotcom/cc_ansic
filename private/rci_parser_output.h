...
static connector_bool_t rci_output_data(rci_buffer_t * const output, char const * const data, size_t const bytes)
{
...
static connector_bool_t rci_output_rcistr(rci_buffer_t * const output, rcistr_t const * const rcistr)
{
    return rci_output_data(output, rcistr->data, rcistr->length);
}

static connector_bool_t rci_output_cstr(rci_buffer_t * const output, char const * const cstr)
{
    ASSERT(cstr != NULL);
...
static connector_bool_t rci_output_formatted(rci_buffer_t * const output, char const * const format, ...)
{
    ...
    va_start(ap, format);
    {
        ...
        ASSERT(result >= 0);
        overflow = connector_bool(need >= have);
        if (!overflow)
        {
...
static connector_bool_t rci_output_non_entity_character(rci_buffer_t * const output, int const value)
{
    connector_bool_t const non_entity_character = connector_bool(rci_entity_name(value) == NULL);
...
static connector_bool_t rci_output_entity_name(rci_buffer_t * const output, int const value)
{
...
static cstr_t const * enum_value_to_cstr(rci_t * const rci, unsigned int const value)
{
    ...
    connector_element_value_limit_t const * const value_limit = element->value_limit;
    connector_element_value_enum_t const * const enum_value = &value_limit->enum_value;
...
static cstr_t const * on_off_value_to_cstr(connector_on_off_t const value)
{
    cstr_t const * result = NULL;
    ...
    case connector_off: result = RCI_OFF;   break;
    case connector_on:  result = RCI_ON;    break;
    }
...
static cstr_t const * boolean_value_to_cstr(connector_boolean_t const value)
{
    cstr_t const * result = NULL;
    ...
    {
    UNHANDLED_CASES_ARE_NEEDED
    case connector_false:   result = RCI_FALSE; break;
    case connector_true:    result = RCI_TRUE;  break;
    }
...
static void rci_generate_output(rci_t * const rci)
{
    ...
    {
        if (rci->status != rci_status_complete)
