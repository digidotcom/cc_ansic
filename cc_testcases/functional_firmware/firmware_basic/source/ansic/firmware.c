...
static connector_callback_status_t app_firmware_download_request(connector_firmware_download_start_t * const download_info)
{
    ...
    if ( download_info->filename != NULL && strlen(download_info->filename) == 0 )
    {
        APP_DEBUG("Filename for the firmware update is NULL\n");
        download_info->filename = "/tmp/test_file_without_name.txt";
    }

    testFileHandler = fopen(download_info->filename,"w");
    if ( testFileHandler != NULL )
    {
        APP_DEBUG("File to save the firmware data was created\n");
    }
    else
    {
        APP_DEBUG("File to save the firmware data was NOT created\n");
    }
    /******** End of Modifications for testing stop callback ********/
...

    {
        ...
        if (image_data->image.bytes_used > 0)
        {
            ...
            while (bytes_avail > 0)
            {
                ...
                if (fseek(testFileHandler, offset, SEEK_SET) < 0)
                {
                    APP_DEBUG("fseek return error %d\n", errno);
                }
                bytes_written = fwrite(ptr, bytes_avail, 1, testFileHandler);
